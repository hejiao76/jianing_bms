        ...
        '{{if data.result && data.result.length>0}}',
            ...
            '<tr data-id="{{item.id}}">',
                ...
                '{{if item.status==1}}',
                    ...
                    '<span data-id="{{item.id}}" data-isFirst="2" class="cp setFirstShow_span_js" style="color:green">正常</span>',
                '{{else }}',
                    '<span data-id="{{item.id}}" data-isFirst="1" class="cp setFirstShow_span_js">禁用</span>',
                '{{/if}}',
                '</td>',
                '<td>',
                    '<img data-id="{{item.id}}" data-gongzhangUrl="{{item.cover}}" data-url="{{item.url || \"\"}}" class="cp editNews_js" src="img/icon3.png" title="编辑">',
